# extractors/vegazone_extractor.py
...
class VegazoneExtractor(BaseExtractor):
    """
    Экстрактор для Vegazone:
    - авторизация через BaseExtractor (валюта подхватывается из ответа логина);
    - депозиты берём с return_raw=True и считаем min_deposit для каждой пары (title, name);
    - recommended берём ТОЛЬКО из депозита;
    - эндпоинты без /en (BaseExtractor сам подберёт рабочий путь при необходимости).
    """
    ...
    def get_payment_and_withdraw_systems(self, current_geo: str) -> Tuple[
        ...
        for item in dep_raw or []:
            try:
                ...
                # не валимся на одиночных кривых записях
                continue
