...
class NeedForSpinExtractor(BaseExtractor):
    ...
    - базовые пути без /en; при 404/других вариантах BaseExtractor сам подберёт рабочий endpoint.
    """
    ...
    def get_payment_and_withdraw_systems(self, current_geo: str) -> Tuple[
        ...
        logging.info("[NeedForSpin] Fetching DEPOSIT methods")
        dep_titles, dep_names, rec_dep, dep_raw = self.fetch_methods(
            self.DEPOSIT_URL, country, return_raw=True
        )

        logging.info("[NeedForSpin] Fetching WITHDRAW methods")
