# extractors/spinlander_extractor.py
from typing import List, Tuple, Set, Dict, Any, Optional
import logging

from .base_extractor import BaseExtractor
from .min_deposit_extractor import compute_min_deposit


class SpinlanderExtractor(BaseExtractor):
    """
    Spinlander extractor:
    - авторизация через BaseExtractor (валюта вытягивается прямо из ответа логина)
    - депозиты tянем с return_raw=True и считаем min_deposit на каждую пару (title, name)
    - recommended берём ТОЛЬКО из депозита
    - эндпоинты без /en; при 404/ошибках BaseExtractor сам подберёт рабочий путь
    """

    DEPOSIT_URL: str  = "/api/v1/model/paysystem/deposit"
    WITHDRAW_URL: str = "/api/v1/model/paysystem/withdraw"

    def __init__(self, login, password, user_agent: Optional[str] = None, base_url: Optional[str] = None):
        super().__init__(base_url, login, password, user_agent)

    def get_payment_and_withdraw_systems(self, current_geo: str) -> Tuple[
        List[Dict[str, Any]],  # deposit_enriched (dicts with min_deposit)
        List[str],             # withdraw titles
        List[str],             # deposit names (for compat)
        List[str],             # withdraw names
        Optional[str],         # currency
        int,                   # deposit_count
        Set[Tuple[str, str]]   # recommended (title, name)
    ]:
        """
        Возвращает кортеж:
          (
            deposit_enriched,        # [{'title','name','min_deposit','currency','min_source'}, ...]
            withdraw_titles,         # как раньше
            deposit_names,           # как раньше (для совместимости)
            withdraw_names,          # как раньше
            self.currency,           # валюта (берётся из логина или из методов)
            self.deposit_count,      # кол-во депозитных методов
            recommended_methods      # {(title, name), ...} ТОЛЬКО из депозита
          )
        """
        country = (current_geo or "").split("_")[0]

        logging.info("[Spinlander] fetching DEPOSIT methods")
        dep_titles, dep_names, rec_dep, dep_raw = self.fetch_methods(
            self.DEPOSIT_URL, country, return_raw=True
        )

        logging.info("[Spinlander] fetching WITHDRAW methods")
        wd_titles, wd_names, _rec_wd, _ = self.fetch_methods(
            self.WITHDRAW_URL, country, return_raw=True
        )

        # Обогащаем депозиты минимумами
        deposit_enriched: List[Dict[str, Any]] = []
        for item in dep_raw or []:
            try:
                title = (item.get("title") or item.get("alias") or item.get("name") or "").strip()
                pm = item.get("paymethods") or {}
                pm_pay = pm.get("paymethod") or {}

                name = (
                    item.get("name")
                    or pm_pay.get("name")
                    or pm_pay.get("code")
                    or item.get("doc_id")
                    or ""
                ).strip()
                if not title or not name:
                    continue

                min_val, source = compute_min_deposit(item)
                cur = ((pm.get("currency") or {}).get("code")) or self.currency

                deposit_enriched.append({
                    "title": title,
                    "name": name,
                    "min_deposit": float(min_val) if min_val is not None else None,
                    "currency": cur,
                    "min_source": source,
                })
            except Exception:
                # не валимся на единичных кривых записях
                continue

        recommended_methods: Set[Tuple[str, str]] = set(rec_dep)

        return (
            deposit_enriched,
            wd_titles,
            dep_names,
            wd_names,
            self.currency,
            self.deposit_count,
            recommended_methods,
        )
